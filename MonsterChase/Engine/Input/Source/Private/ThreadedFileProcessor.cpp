...
namespace Engine
{
	...
	namespace Azra
	{
		...
		ThreadedFileProcessor::LoadTask * ThreadedFileProcessor::GetFromLoadQueue(void)
		{
			...
			//m_LoadQueueSemaphore.Decrement();
		...
		ThreadedFileProcessor::_LoadTaskData * ThreadedFileProcessor::GetFromProcessQueue(void)
		{
			...
			//m_ProcessQueueSemaphore.Decrement();
			...
			while (!bDone)
			{
				...
				else if (LQResult == WAIT_OBJECT_0 + 1)
				{
					...
					if (pLoadTask)
					{
						...
						Azra::ConsolePrint("Processing %s\n", pFileName);
						...
						{
							Azra::ConsolePrint("Error Opening File %s : %d", pFileName, GetLastError());
							delete pLoadTask;
						}
						else
						{
							...
							{
								Azra::ConsolePrint("Failed reading %s : %d", pFileName, GetLastError());
								delete pLoadTask;
							}
							else
							{
								...
								{
									Azra::ConsolePrint("Failed waiting on File I/O %s : %d", pFileName, GetLastError());
									delete pLoadTask;
								}
							}
						}
